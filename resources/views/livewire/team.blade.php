            ...
            <div class="container">
                <div class="row justify-content-center g-4">
                    ...
                    <div class="col-xl-3 col-lg-4 col-md-6">
                        <div class="overflow-hidden text-center border-0 shadow-sm card team-card rounded-4 position-relative">
                            <div class="team-img-wrapper position-relative">
                                @if($team->image)
                                    <img src="{{ asset('storage/' . $team->image) }}" alt="{{ $team->title }}" class="img-fluid team-image w-100">
                                @else
                                    <div class="placeholder d-flex align-items-center justify-content-center bg-light" style="height: 320px;">
                                        <i class="fas fa-user fa-3x text-muted"></i>
                                    </div>
                                @endif

                                <!-- Hover social icons -->
                                <ul class="m-0 social-links list-inline position-absolute top-50 start-50 translate-middle">
                                    @if($team->fb_url)
                                        <li class="list-inline-item">
                                            <a href="{{ $team->fb_url }}" class="social-link facebook"><i class="fab fa-facebook-f"></i></a>
                                        </li>
                                    @endif
                                    @if($team->tk_url)
                                        <li class="list-inline-item">
                                            <a href="{{ $team->tk_url }}" class="social-link tiktok"><i class="fab fa-tiktok"></i></a>
                                        </li>
                                    @endif
                                    @if($team->in_url)
                                        <li class="list-inline-item">
                                            <a href="{{ $team->in_url }}" class="social-link instagram"><i class="fab fa-instagram"></i></a>
                                        </li>
                                    @endif
                                </ul>
                            </div>
                            ...
                            <div class="py-4 card-body">
                                <h5 class="mb-1 fw-bold text-dark">{{ $team->name }}</h5>
